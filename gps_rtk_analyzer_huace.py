...
- PDOP和HDOP值{'远低于' if dop_s['pdop']['mean'] < 3.0 else '接近'}工程阈值，卫星几何构型{'优秀' if dop_s['pdop']['mean'] < 3.0 else '一般'}
- 数据完整性达{stats['data_completeness']:.1f}%，{'无数据间隙' if stats['data_gaps'] == 0 else '存在数据间隙'}
- 接收机位置稳定，观测环境良好

### 注意事项
- R95为{pos_stats['r95']*100:.1f}厘米，{'达到工程阈值（<2cm）' if r95_pass else '略高于理想值（<2cm），但仍属可接受范围'}
